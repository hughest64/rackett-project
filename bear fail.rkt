...
(define (main b)
  (big-bang b                   
            (on-tick   add-bear)    
            (to-draw   render-bears)   
            (on-mouse  handle-mouse)))


;; ListOfString -> ListOfBear
;; produce an additonal bear on MTS where clicked
(check-expect (add-bear empty) 
              empty)
(check-expect (add-bear (cons (make-bear 0 0 0) empty))
...
(check-expect (render-bears empty) 
              MTS)
(check-expect (render-bears (cons (make-bear 20 50 200)
                                 (cons (make-bear 100 200 300)
                                       empty)))
              (place-image (rotate 20 BEAR-IMG) 50 300
                           (place-image (rotate 100 BEAR-IMG) 200 300
                                        MTS)))
...
;; Bear Image -> Image
;; render an image of a bear on a given image
(check-expect (render-bear-on (make-bear 0 0 0) MTS)
              (place-image (rotate 90 BEAR-IMG) 0 0 MTS)) 

;(define (render-bear-on b img) MTS) ;stub

(define (render-bear-on b img)
  (place-image (rotate (bear-s b) BEAR-IMG) (bear-x b) (bear-y b) img))


;; Bear Integer Integer MouseEvent -> ListOfBear
